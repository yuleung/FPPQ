...
if nargout > 1
    ...
    bn3=CODE(513:768,:);
    bn4=CODE(769:1024,:);
    bn5=CODE(1024+1:1024+256,:);
    bn6=CODE(1024+256+1:1024+512,:);
    bn7=CODE(1024+512+1:1024+768,:);
    bn8=CODE(1024+768+1:2048,:);
    ...
    temp21=x_encoding-CODEBOOK(:,index(5,:));
    copy=repmat(subE,[size(temp21,1),1]);
    g(1:256,1024+1:1024+256)=2*W*(W'*x_encoding-Y)*bn5'+2*gama*(x_encoding-P'*XTrain)*bn5'+4*miyou*(copy.*temp21)*bn5';

    %'
    temp21=x_encoding-CODEBOOK(:,index(6,:));
    copy=repmat(subE,[size(temp21,1),1]);
    g(1:256,1024+256+1:1024+512)=2*W*(W'*x_encoding-Y)*bn6'+2*gama*(x_encoding-P'*XTrain)*bn6'+4*miyou*(copy.*temp21)*bn6';

    %'
    temp21=x_encoding-CODEBOOK(:,index(7,:));
    copy=repmat(subE,[size(temp21,1),1]);
    g(1:256,1024+512+1:1024+768)=2*W*(W'*x_encoding-Y)*bn7'+2*gama*(x_encoding-P'*XTrain)*bn7'+4*miyou*(copy.*temp21)*bn7';

    %'
    temp21=x_encoding-CODEBOOK(:,index(8,:));
    copy=repmat(subE,[size(temp21,1),1]);
    g(1:256,1024+768+1:2048)=2*W*(W'*x_encoding-Y)*bn8'+2*gama*(x_encoding-P'*XTrain)*bn8'+4*miyou*(copy.*temp21)*bn8';
